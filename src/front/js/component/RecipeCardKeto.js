...
export default function RecipeCardKeto() {
    const {store, actions} = useContext(Context); //Const to call store data from Flux (Actions is not used yet)
    const [recipes, setKeto] = useState([]); //UseState run the function from recipes (API)
    console.log(recipes, "Recipes Keto")

    useEffect(() => {
        setKeto(store.recipeKeto)
    }, [store.recipeKeto] // In Here we call out again to keep stored the data on re-load the page
    )
            ...
            <div key={i} className="col-sm card m-2 cardShape" style={{minWidth: "18rem"}}>
                <img src={`https://spoonacular.com/recipeImages/${x.id}-556x370.${x.imageType}`} className="card-img-top shapeImageTop" alt="recipe Image" />
                    <div className="card-body text-light">
                        ...
                        <Link to={`/SingleRecipeKeto/${x.id}`} className="btn btn-orange">
                        Read More
                        </Link>
